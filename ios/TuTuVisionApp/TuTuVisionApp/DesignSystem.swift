...
/// 深/浅双模:
/// - 浅模式:奶油白底 + 暖橙/紫 9% 柔光(原观感)
/// - 深模式:近黑底 + 品牌橙/紫 20% 柔光(深色环境下需要更强的点缀才显形)
struct AuroraBackground: View {
    ...
    private var effectiveLeading: Color {
        leadingTint ?? (colorScheme == .dark
            ? TuTuColor.orange.opacity(0.22)
            : TuTuColor.orange.opacity(0.09))
    }

    private var effectiveTrailing: Color {
        trailingTint ?? (colorScheme == .dark
            ? TuTuColor.purple.opacity(0.24)
            : TuTuColor.purple.opacity(0.08))
    }
    ...
    var body: some View {
        TuTuGradient.surface
            .overlay(alignment: .topLeading) {
                Circle()
                    .fill(effectiveLeading)
                    .frame(width: 260, height: 260)
                    .blur(radius: colorScheme == .dark ? 70 : 44)
                    .offset(x: -90, y: -30)
            }
            .overlay(alignment: .topTrailing) {
                Circle()
                    .fill(effectiveTrailing)
                    .frame(width: 280, height: 280)
                    .blur(radius: colorScheme == .dark ? 80 : 48)
                    .offset(x: 100, y: -20)
            }
    }
}
...
/// 业务调用保持不变:`HeroCard(gradient:, height:, corner:, decoration:, content:)`。
/// 内部下沉到 `LiquidGlassCard(style: .hero)`,自动获得:
///   - displayP3 主题渐变 + 双层 Material
///   - 流动光边 + Metal 液态噪声(iOS 17+)
///   - 顶部 1/3 白色高光
///   - 24pt 主题色外阴影
///
/// `gradient` 参数继续保留作视觉一致性 hint,但实际渲染会从中提取色环 → 派生光边色。
struct HeroCard<Decoration: View, Content: View>: View {
    ...
    /// Hero 卡默认使用兔兔品牌环 — 橙/珊瑚/紫。
    /// 调用方仍可以通过 `gradient` 间接传达色调,但最终流动边/外阴影色环由这里管控。
    private var themeColors: [Color] {
        [
            Color(.displayP3, red: 1.00, green: 0.42, blue: 0.21, opacity: 1.0),
            Color(.displayP3, red: 1.00, green: 0.56, blue: 0.39, opacity: 1.0),
            Color(.displayP3, red: 0.48, green: 0.38, blue: 1.00, opacity: 1.0)
        ]
    }
    ...
    var body: some View {
        LiquidGlassCard(
            ...
            useMetalNoise: false,
            accentBlobs: .brandFluid                       // Hero 卡 → 兔兔品牌橙紫流体
        ) {
            ZStack(alignment: .topLeading) {
                // 柔光装饰圆(保留原 hero 的氛围感,但压暗一些让玻璃 blob 主导)
                Circle()
                ...
                content()
                    .foregroundStyle(Color.primary)         // 跟随色板:深紫/深橙玻璃背景 → 近白/近黑
...
/// 历史上的常规卡容器。为了不改动 5000+ 行业务代码,保留原 API,
/// 内部直接下沉到 `LiquidGlassCard`(feature style + premium 玻璃 blob)。
/// - `.solid` → 兔兔橙紫品牌 blob(业务默认,最接近 App 品牌调性)
/// - `.glass` → 蓝调流体 blob(内容型卡,冷静)
/// - `.grouped` → 粉紫流体 blob(列表分组,柔和)
struct PrimaryCard<Content: View>: View {
    ...
    private var blobs: [LiquidGlassBlob] {
        switch style {
        case .solid:   return .brandFluid       // 兔兔橙紫
        case .glass:   return .blueFluid        // 蓝调冷静
        case .grouped: return .pinkPurpleFluid  // 粉紫柔和
        }
    }
}
...
enum TemplateVisualBook {
    static func assetPath(for id: String) -> String? {
        ...
        // 模板广场卡片专用图
        case "plaza-hero":
            return "Templates/plaza-hero-overview.png"
        case "plaza-stat-all":
            return "Templates/plaza-stat-all.png"
        case "plaza-stat-premium":
            return "Templates/plaza-stat-premium.png"
        case "plaza-stat-categories":
            return "Templates/plaza-stat-categories.png"
        case "plaza-stat-collections":
            return "Templates/plaza-stat-collections.png"
        case "plaza-collection-ecommerce":
            return "Templates/plaza-collection-ecommerce.png"
        case "plaza-collection-social":
            return "Templates/plaza-collection-social.png"
        case "plaza-collection-branding":
            return "Templates/plaza-collection-branding.png"
        case "plaza-collection-portrait":
            return "Templates/plaza-collection-portrait.png"
        default:
            return nil
        }
    }
...
struct TemplateArtworkView: View {
    ...
    var body: some View {
        GeometryReader { geo in
                ...
                if let url = TemplateVisualBook.imageURL(for: template),
                   let uiImage = UIImage(contentsOfFile: url.path) {
                    Image(uiImage: uiImage)
                        .resizable()
                        .scaledToFill()
                        .frame(width: geo.size.width, height: geo.size.height)   // 明确尺寸,防止 scaledToFill 溢出父 frame
                        .clipped()
                } else {
            ...
            .frame(width: geo.size.width, height: geo.size.height)               // 强制 ZStack 不溢出
            .clipped()                                                           // 双保险:先 clip 再 shape
