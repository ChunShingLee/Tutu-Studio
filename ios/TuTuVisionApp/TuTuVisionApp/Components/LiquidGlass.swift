...
///
/// v2(2026-05):叠加 `flowingBorder` 流动光边,色环从 palette 派生。
struct GlowingMeshCard<Content: View>: View {
    ...
    var body: some View {
        ...
        .flowingBorder(
            ...
            colors: LiquidGlassMaterial.closedRing(from: Array(palette.colors.prefix(4)))
        )
        .shadow(
            color: (palette.colors.first ?? .orange).opacity(colorScheme == .dark ? 0.32 : 0.22),
...
/// v2(2026-05):叠加 pulse 呼吸光边,单屏多块时动画错峰以降低视觉疲劳。
struct GlowingMeshTile<Content: View>: View {
    ...
    var body: some View {
        ...
        // Tile 尺寸较小 → 静态玻璃边更收敛,动效留给业务按需开启
        .flowingBorder(
            style: .static,
            cornerRadius: cornerRadius,
            colors: LiquidGlassMaterial.closedRing(from: Array(palette.colors.prefix(3)))
        )
        .shadow(color: (palette.colors.first ?? .orange).opacity(0.18), radius: 10, y: 4)
    }
}
