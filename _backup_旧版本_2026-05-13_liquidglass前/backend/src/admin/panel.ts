...
import AdminJS, { ActionContext, CurrentAdmin, ResourceWithOptions } from 'adminjs';
import AdminJSFastify from '@adminjs/fastify';
import { Database, Resource, getModelByName } from '@adminjs/prisma';
import { ContentReviewStatus, TemplateStatus } from '@prisma/client';
...
import {
  getContentStudioPageData,
  getJobCommandPageData,
  getAdminMetrics,
...
import {
  componentLoader,
  contentStudioComponentId,
  dashboardComponentId,
  jobCommandComponentId,
  revenueOpsComponentId
} from './components.js';
      ...
      contentStudio: {
        ...
        handler: async (request, _response, context) => {
          if (request.method === 'post' && request.payload?.action === 'bulkReviewTemplates') {
            ...
            const status = request.payload?.status as ContentReviewStatus | undefined;
            const summary = request.payload?.summary as string | undefined;
      ...
      jobCommand: {
        ...
        handler: async (request, _response, context) => {
          if (request.method === 'post' && request.payload?.action === 'retryJobs') {
            const ids = Array.isArray(request.payload?.ids) ? request.payload.ids : [];
            await Promise.all(ids.map((id: string) => runJobAction(id, 'retry', context.currentAdmin?.id)));
            return {
          ...
          if (request.method === 'post' && request.payload?.action === 'cancelJobs') {
            const ids = Array.isArray(request.payload?.ids) ? request.payload.ids : [];
            await Promise.all(ids.map((id: string) => runJobAction(id, 'cancel', context.currentAdmin?.id)));
            return {
              ...(await getJobCommandPageData()),
              notice: {
...
async function authenticateAdmin(email: string, password: string): Promise<AuthenticatedAdmin | null> {
...
function guardedAction(permission: string) {
  return {
    isAccessible: ({ currentAdmin }: ActionContext) => hasPermission(currentAdmin, permission)
  };
}
...
function readOnlyActions(readPermission: string) {
  return {
    new: hiddenAction(),
    edit: hiddenAction(),
    delete: hiddenAction(),
...
function makeTemplateStatusAction(
  name: string,
  label: string,
  icon: string,
  variant: 'success' | 'info' | 'danger' | 'primary',
...
function makePaymentOrderAction(
  name: string,
  label: string,
  icon: string,
  variant: 'success' | 'info' | 'danger' | 'primary',
...
function makeContentReviewAction(
  name: string,
  label: string,
  icon: string,
  variant: 'success' | 'info' | 'danger' | 'primary',
  guard: string,
  entityType: 'ASSET' | 'COMMUNITY_POST',
...
function makeRecordAction(configAction: {
  ...
  guard: string;
  permission: string;
  handler: (args: { id: string; currentAdmin?: AuthenticatedAdmin }) => Promise<{ message: string; type: 'success' | 'info' | 'error' }>;
}) {
  return {
    ...
    isAccessible: ({ currentAdmin }: ActionContext) => hasPermission(currentAdmin, configAction.permission),
    handler: async (request: { method: 'get' | 'post' }, _response: unknown, context: ActionContext) => {
      ...
      const notice = await configAction.handler({
        id: recordId,
        currentAdmin: context.currentAdmin as AuthenticatedAdmin | undefined
      });
...
function hasPermission(currentAdmin: CurrentAdmin | undefined, permission: string) {
  const admin = currentAdmin as AuthenticatedAdmin | undefined;
