...
struct CreateView: View {
    ...
    private var workflowSummaryCard: some View {
        ...
        .overlay {
            RoundedRectangle(cornerRadius: 24, style: .continuous)
                .stroke(createMode.accent.opacity(0.10), lineWidth: 1)
        }
    }
    ...
    private func workflowStep(index: Int, label: String, title: String, subtitle: String, palette: AnimatedMeshBackground.Palette) -> some View {
        GlowingMeshTile(palette: palette, cornerRadius: 20, padding: 14) {
    ...
    private var headerCard: some View {
        ZStack {
            // 底图 + 双层暗化：上层整体压黑保住 metric 行、下层更重保住大标题
            headerArtwork
                .overlay {
                    LinearGradient(
            ...
            VStack(alignment: .leading, spacing: 10) {
                // 模式 Capsule：只保留一个信息主导的元素，去掉之前的 eyebrow 堆叠
                HStack(spacing: 8) {
                    ...
                    if createMode != .textToImage {
                        Text(headerReferenceStatus)
                            ...
                            .foregroundStyle(hasReferenceImage ? .white : .white.opacity(0.9))
                            .padding(.horizontal, 10)
                            .padding(.vertical, 6)
                            .background(
                                (hasReferenceImage ? Color(red: 0.18, green: 0.64, blue: 0.46) : Color.black.opacity(0.26))
                                    .opacity(hasReferenceImage ? 0.85 : 1),
                                in: Capsule()
                            )
                    }
                ...
                // Metric 行：用一个统一的 glass capsule 容器承载 3 个信息，避免零散叠加
                HStack(spacing: 8) {
                    LiquidGlassMetric(title: style, systemImage: "camera.filters")
                    LiquidGlassMetric(title: aspectRatio, systemImage: "aspectratio")
                    LiquidGlassMetric(title: headerGuideMetric, systemImage: headerGuideSymbol)
                }
            }
            .padding(.horizontal, 20)
            .padding(.vertical, 20)
    ...
    private var headerSubtitle: String {
        if let selectedTemplate {
            return "当前已套用“\(selectedTemplate.title)”模板，你可以继续细化 Prompt 和参数。"
        }

        if createMode == .textToImage {
            return createMode.heroSubtitle
        }

        if hasReferenceImage {
            return selectedUIImage != nil
                ? "正在使用你上传的图片作为参考，接下来重点描述要保留的结构和想升级的氛围。"
                : "当前主视觉已切到参考素材，继续补充你想保留的主体关系和新的成片方向即可。"
        }
    ...
    private var headerGuideMetric: String {
        switch createMode {
        case .textToImage:
            return "可直出首稿"
        case .imageToImage:
            return hasReferenceImage ? "延展已就绪" : "先选参考图"
        case .localEdit:
            return hasReferenceImage ? "可做局部修改" : "先选待修改图"
        }
    }

    private var headerGuideSymbol: String {
        switch createMode {
        case .textToImage:
            return "sparkles"
        case .imageToImage:
            return hasReferenceImage ? "checkmark.circle.fill" : "photo"
        case .localEdit:
            return hasReferenceImage ? "paintbrush.pointed.fill" : "photo"
        }
    }
        ...
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                ...
                Text(createMode == .textToImage ? "可选" : "必选")
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(createMode == .textToImage ? .secondary : TuTuColor.orange)
                    .padding(.horizontal, 10)
                    .padding(.vertical, 6)
    ...
    private var demoMaterialCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    cardTitle("演示照片素材")
                    Text("先放一组更接近真实商业成片的素材，直接演示图生图与局部修改效果。")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Spacer()
                        ...
                        Button {
                            ...
                            VStack(alignment: .leading, spacing: 12) {
                                ZStack(alignment: .topLeading) {
                                    ...
                                    Text(selectedDemoMaterial?.id == material.id ? "当前使用中" : "一键套用")
                                        .font(.caption2.weight(.semibold))
                                        .foregroundStyle(selectedDemoMaterial?.id == material.id ? .white : TuTuColor.dark)
                                        .padding(.horizontal, 10)
                                        .padding(.vertical, 6)
                                        .background(selectedDemoMaterial?.id == material.id ? AnyShapeStyle(material.gradient) : AnyShapeStyle(.ultraThinMaterial), in: Capsule())
                                        .padding(12)
                                }
                                ...
                                VStack(alignment: .leading, spacing: 6) {
                                    ...
                                    Text(material.subtitle)
                                        ...
                                        .foregroundStyle(.secondary)
                                        .lineLimit(2)
                                    Text(material.sceneTag)
                                        .font(.caption2.weight(.semibold))
                                        .foregroundStyle(material.emphasis)
                                }
                            }
                            .padding(14)
                            ...
                            .overlay {
                                RoundedRectangle(cornerRadius: 26, style: .continuous)
                                    .stroke(selectedDemoMaterial?.id == material.id ? material.emphasis.opacity(0.38) : Color.black.opacity(0.03), lineWidth: selectedDemoMaterial?.id == material.id ? 2 : 1)
                            }
                        }
                        .buttonStyle(.plain)
                    }
            ...
            HStack(alignment: .top) {
                ...
                VStack(alignment: .trailing, spacing: 4) {
                    Text("\(cleanedPrompt.count) 字")
                        .font(.caption.weight(.semibold))
                        .foregroundStyle(cleanedPrompt.count > 80 ? TuTuColor.orange : .secondary)
    ...
    private var parametersCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack(spacing: 12) {
                parameterSummaryTile(title: "当前风格", value: style, emphasis: TuTuColor.orange)
                parameterSummaryTile(title: "输出比例", value: aspectRatio, emphasis: TuTuColor.purple)
            }
        }
        .padding(18)
        .background(.background, in: RoundedRectangle(cornerRadius: 24, style: .continuous))
    ...
    private func parameterSummaryTile(title: String, value: String, emphasis: Color) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(title)
                .font(.caption)
                .foregroundStyle(.secondary)
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(14)
        .background(emphasis.opacity(0.08), in: RoundedRectangle(cornerRadius: 18, style: .continuous))
    }
            ...
            Text(title)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 16, style: .continuous)
                        .fill(isSelected ? AnyShapeStyle(LinearGradient(colors: [TuTuColor.orange, TuTuColor.purple], startPoint: .topLeading, endPoint: .bottomTrailing)) : AnyShapeStyle(TuTuColor.groupedBackground))
                )
        }
        .buttonStyle(.plain)
    }
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 6) {
                Text(mode.badgeDisplayTitle)
                    .font(.caption2.weight(.semibold))
                    .foregroundStyle(isSelected ? mode.accent : .secondary)
                Spacer()
