...
import { serializeAdminUser, serializeAsset, serializeAuditLog, serializeCommunityPost, serializeContentReview, serializeJob, serializeOrder, serializePlan, serializeSubscription, serializeTemplate, serializeUser } from '../lib/serializers.js';
...
export async function updateUser(id, input, actorAdminId) {
    const data = {};
    if (typeof input.nickname === 'string')
        data.nickname = input.nickname;
    if (typeof input.planCode === 'string')
        data.planCode = input.planCode;
    if (typeof input.quotaRemaining === 'number')
        data.quotaRemaining = input.quotaRemaining;
...
export async function createTemplate(input, actorAdminId) {
...
export async function updateTemplate(id, input, actorAdminId) {
    const data = { updatedByAdminId: actorAdminId ?? null };
    ...
    if (typeof input.promptHint === 'string')
        data.promptHint = input.promptHint;
...
export async function updateTemplateStatus(id, status, actorAdminId) {
    const template = await prisma.template.update({
        where: { id },
        data: {
            status,
...
export async function updateAdminSettings(input, actorAdminId) {
...
export async function runJobAction(id, action, actorAdminId) {
    if (action === 'retry') {
...
export async function createPlan(input, actorAdminId) {
...
export async function updatePlan(id, input, actorAdminId) {
    const data = {};
    if (typeof input.name === 'string')
        data.name = input.name;
    if (typeof input.priceCents === 'number')
        data.priceCents = input.priceCents;
    if (typeof input.quotaDaily === 'number')
        data.quotaDaily = input.quotaDaily;
...
export async function updateSubscription(id, input, actorAdminId) {
    const data = {};
    if (typeof input.status === 'string')
        data.status = input.status;
    if (typeof input.renewalAt === 'string')
        data.renewalAt = new Date(input.renewalAt);
    if (input.renewalAt === null)
        data.renewalAt = null;
    if (typeof input.endAt === 'string')
        data.endAt = new Date(input.endAt);
    if (input.endAt === null)
        data.endAt = null;
...
export async function updatePaymentOrder(id, input, actorAdminId) {
    const data = {};
    if (typeof input.status === 'string')
        data.status = input.status;
    ...
    if (input.channel === null)
        data.channel = null;
    if (typeof input.paidAt === 'string')
        data.paidAt = new Date(input.paidAt);
...
export async function reviewContentItem(entityType, entityId, input, actorAdminId) {
...
export async function reviewTemplatesBulk(ids, input, actorAdminId) {
    if (!ids.length) {
...
export async function getContentStudioPageData() {
    ...
    return {
        ...
        queue: queueData.queue.slice(0, 8).map(item => ({
            ...item,
            previewImageUrl: item.entityType === 'TEMPLATE'
...
function latestReviewSummary(reviews, entityType, entityId) {
    return reviews.find(review => review.entityType === entityType && review.entityId === entityId)?.summary ?? null;
}
function mapReviewStatusToTemplateStatus(status) {
    switch (status) {
        case 'APPROVED':
            return 'PUBLISHED';
        case 'REJECTED':
...
async function loadTemplatePreviewMap(templateIds) {
...
function buildDailyTrend(jobs, since) {
...
function buildRevenueTrend(orders) {
    ...
    const previousMap = new Map(currentDays.map((day, index) => {
...
function normalizeFailureReason(message) {
    ...
    if (text.includes('timeout') || text.includes('超时'))
        return '上游响应超时';
    if (text.includes('quota') || text.includes('rate limit') || text.includes('额度'))
        return '额度或频率限制';
    if (text.includes('reference image') || text.includes('fetch reference'))
        return '参考图处理失败';
    if (text.includes('returned no image'))
        return '模型未返回图片';
