...
export interface ImageGenerationInput {
  ...
  mode?: 'textToImage' | 'imageToImage' | 'localEdit';
...
interface FigureScriptResponse {
...
async function runFigureImageScript(input: ImageGenerationInput, prompt: string): Promise<FigureScriptResponse> {
...
async function writeReferenceImage(filePath: string, input: ImageGenerationInput): Promise<void> {
...
function parseScriptResponse(stdout: string, stderr: string): FigureScriptResponse {
  ...
  try {
    return JSON.parse(trimmed) as FigureScriptResponse;
  } catch (error) {
